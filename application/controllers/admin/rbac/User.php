...
class User extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->session->isLoggedIn) {
            ...
            $this->lang->load('admin/rbac/user_lang');
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => 'index.php/admin/rbac/user'
        );
        ...
        foreach ($users as $user) {
            $data['users'][] = array(
                ...
                'edit' => 'index.php/admin/rbac/user/edit?user_id=' . $user['user_id']
            );
        }
        $data['selected'] = array();
        $data['add'] = 'index.php/admin/rbac/user/add';
        $data['delete'] = 'index.php/admin/rbac/user/delete';
        ...
        $this->load->view('admin/rbac/user', $data);
        // set footer
...
//            $this->form_validation->set_rules('image', 'Image', 'trim|required');
            ...
            if($this->form_validation->run() && $this->input->post()) { //post the changed information
                ...
                if ($this->model_user->addUser($post)) {
                    redirect('admin/rbac/user?success=1');
                } else {
                    redirect('admin/rbac/user/add');
                }
                ...
                $this->load->view('admin/rbac/user_form', $data);
            }
        } else {
            redirect('admin/rbac/user?success=3');
        }
    }
    ...
    private function _form($action, $user_id = '')
    {
        ...
        $data['action'] = 'index.php/admin/rbac/user/' . $action . '?user_id=' . $user_id;
        $data['cancel'] = 'index.php/admin/rbac/user';
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => 'index.php/admin/rbac/user/' . $action
        );
...
//            $this->form_validation->set_rules('image', 'Image', 'trim|required');
            ...
            if($this->form_validation->run() && $this->input->post()) { //post the changed information
                ...
                if ($this->model_user->editUser($post)){
                    ...
                    redirect('admin/rbac/user/edit?user_id='.$user_id);
                }
            } else { // render the form
    ...
    public function delete()
    {
        if ($this->_checkAuthentication()) {
            if($this->_validateDelete() && $this->input->post('selected')){
                ...
                redirect('admin/rbac/user?success=2');
            }
        } else {
            redirect('admin/rbac/user?success=3');
        }
